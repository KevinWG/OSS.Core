...
interface TableAccessButtonProps<T> {
  record: T;
  /**
   * 如果涉及服务端接口交互，实现此方法即可
   */
...
// 锁定按钮
export function TableAccessButton<T>(props: TableAccessButtonProps<T>) {
  ...
  return <AccessButton {...aBtnProps}>{btn_text}</AccessButton>;
}

interface TableAccessButtonsProps<T extends BaseMo> {
  record: T;
  children?: React.ReactNode;
  condition_buttons?: {
    condition: (r: T) => boolean;
    buttons: Omit<TableAccessButtonProps<T>, 'callback' | 'record' | 'fetchKey'>[];
  }[];
  fetchKey?: (record: T) => string;
  callback?: (res: Resp, item: T, actionName?: string) => void;
}
export default function TableAccessButtons<T extends BaseMo>(
  props: TableAccessButtonsProps<T>,
): JSX.Element {
