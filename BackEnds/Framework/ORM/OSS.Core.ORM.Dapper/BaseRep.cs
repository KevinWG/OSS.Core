...
{
    public abstract class BaseRep<TRep, TType, IdType> : SingleInstance<TRep>, IRepository<TType, IdType> where TRep : class, new()
        where TType : BaseMo<IdType>, new()
    {
        ...
        public virtual async Task<Resp<IdType>> Add(TType mo)
        {
            var res = await ExecuteWriteAsync(async con =>
            {
                ...
                return row > 0 ? new Resp<IdType>() : new Resp<IdType>().WithResp(RespTypes.OperateFailed, "添加失败!");
            });
            if (res.IsSuccess())
            {
                res.data = mo.id;
            ...
            string? totalSql = null)
        {
            return await ExecuteReadSubAsync(async con =>
            {
                ...
                if (!string.IsNullOrEmpty(totalSql))
                {
                    ...
                    if (total <= 0) return new PageListResp<RType>().WithResp(RespTypes.OperateObjectNull, "没有查到相关信息！");
                }

                var list = (await con.QueryAsync<RType>(selectSql, paras)).ToList();
                return list.Count==0 ? new PageListResp<RType>().WithResp(RespTypes.OperateObjectNull, "没有查到相关信息！") : new PageListResp<RType>(total, list.ToList());
            });
        }
        ...
        protected Task<Resp<RespParaType>> ExecuteReadAsync<RespParaType>(Func<IDbConnection, Task<RespParaType>> func)
            => ExecuteAsync(async con =>
            {
                ...
                return res != null
                    ...
                    : new Resp<RespParaType>().WithResp(RespTypes.OperateObjectNull, "未发现相关数据！");
            }, false);
