...
internal class CreateActivity : BasePassiveActivity<CreateReq, LongResp, CreateContext>
{
    ...
    protected override async Task<TrafficSignal<LongResp, CreateContext>> Executing(CreateReq req)
    {
        ...
        return !addRes.IsSuccess()
            ...
            : new TrafficSignal<LongResp, CreateContext>(new LongResp(addRes.data.id),
                new CreateContext(addRes.data, req.manual > 0));
    }

    private static async Task<IResp<FlowNodeMo>> AddFlow(CreateReq req)
    {
        var pipelineRes = await InsContainer<IPipelineCommon>.Instance.GetLine(req.pipeline_id);
        ...
        flow.status    = req.manual > 0 ? ProcessStatus.Waiting : ProcessStatus.Processing;

        flow.FormatBaseByContext();

        await InsContainer<IFlowCommon>.Instance.AddNode(flow);
